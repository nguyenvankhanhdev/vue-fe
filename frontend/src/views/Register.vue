        ...
        <div class="register-card">
          ...
          <form @submit.prevent="handleRegister" class="register-form" novalidate>
            ...
            <div class="form-group">
              ...
              <div class="password-input">
                <input
                  ...
                  @input="clearErrors"
                >
                <button type="button" class="password-toggle" @click="toggleConfirmPassword" aria-label="Hiện/ẩn xác nhận mật khẩu">
                  <i :class="showConfirmPassword ? 'fas fa-eye-slash' : 'fas fa-eye'"></i>
                </button>
              </div>
              <p v-if="registerForm.confirmPassword && !passwordsMatch" class="error-message">Mật khẩu xác nhận không khớp</p>
            </div>
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input 
                  ...
                  required
                  :class="{ 'error': hasFieldError('birth_date') }"
                  @change="clearErrors"
                >
            ...
            <div class="social-register">
              <button type="button" class="social-btn google-btn"><i class="fab fa-google"></i>Đăng ký với Google</button>
              <button type="button" class="social-btn facebook-btn"><i class="fab fa-facebook-f"></i>Đăng ký với Facebook</button>
            </div>
...
const handleRegister = async () => {
  ...
  try {
    ...
    const userData = {
      ...
      last_name: registerForm.value.lastName.trim(),
    ...
    // Cập nhật auth state global
    setAuthState(response.token, response.user)
    
    // Hiển thị thông báo thành công
    success(`Chào mừng ${response.user.first_name}! Tài khoản của bạn đã được tạo thành công.`, 6000)
...
.register-illustration{
  background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);
  display:flex;align-items:center;justify-content:center;color:#fff;text-align:center;padding:60px 40px
}
.illustration-content h3{font-size:28px;font-weight:700;margin-bottom:16px}
.illustration-content p{font-size:16px;opacity:.9;margin-bottom:40px;line-height:1.6}
.benefits{display:flex;flex-direction:column;gap:20px}
.benefit{display:flex;align-items:center;gap:12px;font-size:14px}
.benefit i{font-size:18px;opacity:.85}
...
.register-header h2{font-size:28px;font-weight:700;color:#1f2937;margin-bottom:8px}
...
.form-group input,.form-group select{
  ...
  transition:all .2s ease;background:#f9fafb
}
.form-group input:focus,.form-group select:focus{
  outline:none;border-color:#3b82f6;background:#fff;box-shadow:0 0 0 3px rgba(59,130,246,.12)
}
...
.strength-fill{height:100%;transition:all .25s ease}
...
.strength-text{font-size:12px;color:#6b7280}
...
.form-options{margin:10px 0 22px}
.checkbox{
  display:flex;align-items:flex-start;gap:10px;margin-bottom:12px;cursor:pointer;user-select:none
}
.checkbox input{position:absolute;opacity:0;width:0;height:0}
.checkbox .box{
  width:18px;height:18px;border:2px solid #cbd5e1;border-radius:6px;background:#fff;display:inline-block;
  transition:all .15s ease;position:relative;flex:0 0 18px;margin-top:2px
}
.checkbox input:focus + .box{box-shadow:0 0 0 3px rgba(59,130,246,.15)}
.checkbox input:checked + .box{
  background:#3b82f6;border-color:#3b82f6;
}
.checkbox input:checked + .box::after{
  content:"";position:absolute;left:4px;top:1px;width:6px;height:10px;border:2px solid #fff;
  border-top:none;border-left:none;transform:rotate(45deg)
}
.checkbox .text{font-size:13px;color:#6b7280;line-height:1.45}
.checkbox a{color:#3b82f6;text-decoration:none}
.checkbox a:hover{text-decoration:underline}

/* Buttons */
.register-btn{
  width:100%;padding:14px;background:#3b82f6;color:#fff;border:none;border-radius:10px;
  font-size:16px;font-weight:600;cursor:pointer;transition:all .2s ease;display:flex;align-items:center;justify-content:center;gap:8px
}
.register-btn:hover:not(:disabled){background:#2563eb;transform:translateY(-1px)}
...
.divider span{background:#fff;padding:0 16px;color:#6b7280;font-size:14px}
.social-register{display:flex;gap:12px;margin-bottom:20px}
.social-btn{
  flex:1;padding:10px;border:2px solid #e5e7eb;border-radius:10px;background:#fff;font-size:13px;font-weight:500;cursor:pointer;
  transition:all .2s ease;display:flex;align-items:center;justify-content:center;gap:8px
}
.google-btn:hover{border-color:#ea4335;color:#ea4335}
.facebook-btn:hover{border-color:#1877f2;color:#1877f2}
.login-prompt{text-align:center}
.login-prompt p{color:#6b7280;margin:0;font-size:14px}
.login-link{color:#3b82f6;text-decoration:none;font-weight:600}
.login-link:hover{text-decoration:underline}

/* Responsive */
@media (max-width: 992px){
  .register-wrapper{grid-template-columns:1fr}
  .register-illustration{order:-1;padding:40px 24px}
}
